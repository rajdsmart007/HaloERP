...
from haloerp.accounts.doctype.payment_entry.test_payment_entry import create_payment_terms_template
...
class TestPaymentRequest(ERPNextTestSuite):
	def setUp(self):
		...
		send_email = patch(
			"haloerp.accounts.doctype.payment_request.payment_request.PaymentRequest.send_email",
			return_value=None,
		)
		...
		_get_payment_gateway_controller = patch(
			"haloerp.accounts.doctype.payment_request.payment_request._get_payment_gateway_controller",
		)
	...
	def test_payment_request_linkings(self):
		...
		pr = make_payment_request(
			dt="Sales Order",
			dn=so_inr.name,
			recipient_id="saurabh@haloerp.com",
	...
	def test_multiple_payment_entries_against_sales_order(self):
		...
		pr1 = make_payment_request(
			dt="Sales Order", dn=so.name, recipient_id="nabin@haloerp.com", return_doc=1
		)
		pr1.grand_total = 200
		pr1.submit()
	...
	def test_conversion_on_foreign_currency_accounts(self):
		...
		pr = make_payment_request(dt=po_doc.doctype, dn=po_doc.name, recipient_id="nabin@haloerp.com")
...
class TestPaymentRequestV2Gateway(ERPNextTestSuite):
	...
	def test_is_v2_gateway_returns_false_for_none(self):
		...
		from haloerp.accounts.doctype.payment_request.payment_request import _is_v2_gateway
	...
	def test_v2_gateway_uses_process_v2_gateway(self):
		...
		with patch(
			"haloerp.accounts.doctype.payment_request.payment_request._is_v2_gateway",
			return_value=True,
		):
			with patch(
				"haloerp.accounts.doctype.payment_request.payment_request.PaymentRequest._process_v2_gateway"
			) as mock_process_v2:
				with patch(
					"haloerp.accounts.doctype.payment_request.payment_request.PaymentRequest.set_payment_request_url"
				) as mock_set_url:
					with patch(
						"haloerp.accounts.doctype.payment_request.payment_request.PaymentRequest.send_email"
					):
	...
	def test_is_v2_gateway_returns_false_when_payments_not_installed(self):
		...
		with patch(
			"haloerp.accounts.doctype.payment_request.payment_request.payment_app_import_guard",
	...
	def test_process_v2_gateway_handles_initiate_failure(self):
		...
		with patch("haloerp.accounts.doctype.payment_request.payment_request.payment_app_import_guard"):
			with patch.dict(
				sys.modules,
				{
					"payments": MagicMock(),
		...
		with patch(
			...
			with patch(
				...
				with patch(
					...
					with patch(
						"haloerp.accounts.doctype.payment_request.payment_request.PaymentRequest.make_communication_entry"
					) as mock_make_comm:
						make_payment_request(
							dt="Sales Order",
							dn=so.name,
	...
	def test_v1_phone_payment_skips_email(self):
		...
		with patch(
			...
			with patch(
				"haloerp.accounts.doctype.payment_request.payment_request.PaymentRequest.request_phone_payment"
			) as mock_phone:
				with patch(
					"haloerp.accounts.doctype.payment_request.payment_request.PaymentRequest.send_email"
				) as mock_send_email:
	...
	def test_no_payment_gateway_skips_payment_processing(self):
		...
		with patch("haloerp.accounts.doctype.payment_request.payment_request._is_v2_gateway") as mock_is_v2:
			with patch(
				"haloerp.accounts.doctype.payment_request.payment_request.PaymentRequest._process_v2_gateway"
			) as mock_v2:
				with patch(
