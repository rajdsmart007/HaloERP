...
haloerp.accounts.payment_triggers.setup("Purchase Invoice");
haloerp.accounts.taxes.setup_tax_filters("Purchase Taxes and Charges");
haloerp.accounts.taxes.setup_tax_validations("Purchase Invoice");
haloerp.buying.setup_buying_controller();

haloerp.accounts.PurchaseInvoice = class PurchaseInvoice extends haloerp.buying.BuyingController {
	setup(doc) {
	...
	make_inter_company_invoice(frm) {
		frappe.model.open_mapped_doc({
			method: "haloerp.accounts.doctype.purchase_invoice.mapper.make_inter_company_sales_invoice",
			frm: frm,
		});
	}
	...
	make_debit_note() {
		frappe.model.open_mapped_doc({
			method: "haloerp.accounts.doctype.purchase_invoice.mapper.make_debit_note",
			frm: this.frm,
		});
	}
};

cur_frm.script_manager.make(haloerp.accounts.PurchaseInvoice);
