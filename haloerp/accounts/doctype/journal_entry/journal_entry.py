...
import haloerp
from haloerp.accounts.doctype.invoice_discounting.invoice_discounting import (
	get_party_account_based_on_invoice_discounting,
)

# Re-exported so existing call paths (including custom apps) referencing
# haloerp.accounts.doctype.journal_entry.journal_entry.<fn> keep working.
from haloerp.accounts.doctype.journal_entry.mapper import (
...
from haloerp.accounts.doctype.tax_withholding_entry.tax_withholding_entry import JournalTaxWithholding
from haloerp.accounts.party import get_party_account
from haloerp.accounts.services.gl_validator import validate_opening_entry_against_pcv
...
from haloerp.setup.utils import get_exchange_rate as _get_exchange_rate
...
class JournalEntry(AccountsController):
	...
	if TYPE_CHECKING:
		...
		from haloerp.accounts.doctype.journal_entry_account.journal_entry_account import JournalEntryAccount
	...
	def validate(self):
		...
		from haloerp.accounts.doctype.journal_entry.services.asset_service import AssetService
		from haloerp.accounts.doctype.journal_entry.services.reference_validator import (
	...
	def validate_company_for_periodic_accounting(self):
		if haloerp.is_perpetual_inventory_enabled(self.company):
			frappe.throw(
				_(
	...
	def validate_stock_accounts(self):
		if (
			not haloerp.is_perpetual_inventory_enabled(self.company)
	...
	def build_gl_map(self):
		from haloerp.accounts.doctype.journal_entry.services.gl_composer import JournalEntryGLComposer

		return JournalEntryGLComposer(self).compose()

	def make_gl_entries(self, cancel: int = 0, adv_adj: int = 0) -> None:
		from haloerp.accounts.general_ledger import make_gl_entries
		...
		if not blank_row:
			blank_row = self.append(
				"accounts",
				{
					...
					"cost_center": haloerp.get_default_cost_center(self.company),
				},
			)
...
def _get_invoice_outstanding(doctype: str, docname: str, company: str, account_currency: str | None) -> dict:
	...
	company_currency = haloerp.get_company_currency(company)
