...
frappe.provide("haloerp.accounts");
frappe.provide("haloerp.journal_entry");
...
frappe.ui.form.on("Journal Entry", {
	...
	onload(frm) {
		haloerp.journal_entry.load_defaults(frm);
		haloerp.journal_entry.setup_queries(frm);
		haloerp.accounts.dimensions.setup_dimension_filters(frm, frm.doctype);
	},

	refresh(frm) {
		if (frm.doc.reversal_of && (frm.is_new() || frm.doc.docstatus == 0)) {
			haloerp.journal_entry.lock_reversal_entry(frm);
		}

		haloerp.toggle_naming_series();
		haloerp.journal_entry.add_custom_buttons(frm);
		haloerp.journal_entry.toggle_fields_based_on_currency(frm);
		haloerp.accounts.unreconcile_payment.add_unreconcile_btn(frm);

		if (frm.doc.voucher_type !== "Exchange Gain Or Loss") {
			(frm.doc.accounts || []).forEach((row) =>
				haloerp.journal_entry.set_exchange_rate(frm, row.doctype, row.name)
			);
		}
	},
	...
	company(frm) {
		...
		haloerp.utils.set_letter_head(frm);
	...
	voucher_type(frm) {
		...
		return frappe.call({
			method: "haloerp.accounts.doctype.journal_entry.journal_entry.get_default_bank_cash_account",
			args: {
			...
			callback: ({ message }) => {
				if (message && !$.isEmptyObject(message)) {
					haloerp.journal_entry.update_jv_details(frm, [message]);
				}
			},
		});
	},
	...
	from_template(frm) {
		...
		frappe.db.get_doc("Journal Entry Template", frm.doc.from_template).then((template) => {
			...
			haloerp.journal_entry.update_jv_details(frm, template.accounts);
		});
	},
	...
	get_balance(frm) {
		haloerp.journal_entry.update_totals(frm);
...
frappe.ui.form.on("Journal Entry Account", {
	party(frm, cdt, cdn) {
		...
		return frm.call({
			method: "haloerp.accounts.doctype.journal_entry.journal_entry.get_party_account_and_currency",
			child: row,
	...
	account(frm, cdt, cdn) {
		haloerp.journal_entry.set_account_details(frm, cdt, cdn);
	},

	debit_in_account_currency(frm, cdt, cdn) {
		haloerp.journal_entry.set_exchange_rate(frm, cdt, cdn);
	},
	...
	exchange_rate(frm, cdt, cdn) {
		const row = frappe.get_doc(cdt, cdn);
		const company_currency = haloerp.get_currency(frm.doc.company);
		if (row.account_currency == company_currency || !frm.doc.multi_currency) {
			frappe.model.set_value(cdt, cdn, "exchange_rate", 1);
		}
		haloerp.journal_entry.set_debit_credit_in_company_currency(frm, cdt, cdn);
	},
	...
	reference_name(frm, cdt, cdn) {
		...
		if (row.reference_type === "Purchase Invoice" && !flt(row.debit)) {
			haloerp.journal_entry.get_outstanding(frm, "Purchase Invoice", row.reference_name, row);
		} else if (row.reference_type === "Sales Invoice" && !flt(row.credit)) {
			haloerp.journal_entry.get_outstanding(frm, "Sales Invoice", row.reference_name, row);
		} else if (row.reference_type === "Journal Entry" && !flt(row.credit) && !flt(row.debit)) {
			haloerp.journal_entry.get_outstanding(frm, "Journal Entry", row.reference_name, row);
		}
	},
	...
	accounts_add(frm, cdt, cdn) {
		...
		haloerp.journal_entry.set_balancing_amount(row, frm.doc.difference);
		haloerp.journal_entry.update_totals(frm);
		haloerp.accounts.dimensions.copy_dimension_from_first_row(frm, cdt, cdn, "accounts");
	},
...
Object.assign(haloerp.journal_entry, {
	load_defaults(frm) {
	...
	add_custom_buttons(frm) {
		if (frm.doc.docstatus > 0) {
			frm.add_custom_button(
				__("Ledger"),
				() => haloerp.journal_entry.show_general_ledger(frm),
				__("View")
			);
		}
		...
		if (frm.doc.docstatus == 1 && !frm.doc.reversal_of) {
			frm.add_custom_button(
				...
				() => haloerp.journal_entry.reverse_journal_entry(frm),
				__("Actions")
			);
		}

		if (frm.doc.__islocal) {
			frm.add_custom_button(__("Quick Entry"), () => haloerp.journal_entry.quick_entry(frm));
		}
		...
		if (
			...
			frm.add_custom_button(
				...
				() => haloerp.journal_entry.make_inter_company_journal_entry(frm),
				__("Make")
			);
		}
	},
	...
	make_inter_company_journal_entry(frm) {
		...
		dialog.set_primary_action(__("Create"), () => {
			dialog.hide();
			frappe.call({
				method: "haloerp.accounts.doctype.journal_entry.mapper.make_inter_company_journal_entry",
				args: {
					name: frm.doc.name,
	...
	reverse_journal_entry(frm) {
		frappe.model.open_mapped_doc({
			method: "haloerp.accounts.doctype.journal_entry.mapper.make_reverse_journal_entry",
			frm: frm,
		});
	},
	...
	quick_entry(frm) {
		...
		const dialog = new frappe.ui.Dialog({
			...
			fields: [
				{ fieldtype: "Currency", fieldname: "debit", label: __("Amount"), reqd: 1 },
				{
					...
					reqd: 1,
					options: "Account",
					get_query: () => haloerp.journal_entry.account_query(frm),
				},
				{
					fieldtype: "Link",
		...
		dialog.set_primary_action(__("Save"), () => {
			haloerp.journal_entry.save_quick_entry(frm, dialog.get_values());
			dialog.hide();
		});
		dialog.show();
	},
	...
	save_quick_entry(frm, values) {
		...
		haloerp.journal_entry.add_quick_entry_row(
			frm,
	...
	get_outstanding(frm, reference_type, reference_name, child) {
		return frappe.call({
			method: "haloerp.accounts.doctype.journal_entry.journal_entry.get_outstanding",
			args: {
	...
	set_account_details(frm, cdt, cdn) {
		const row = frappe.get_doc(cdt, cdn);
		if (!row.account) {
			haloerp.journal_entry.clear_fields(frm, cdt, cdn);
			return;
		}
		...
		return frappe.call({
			method: "haloerp.accounts.doctype.journal_entry.journal_entry.get_account_details_and_party_type",
			args: {
				account: row.account,
			...
			callback: ({ message }) => {
				if (!message) return;
				$.extend(row, message);
				haloerp.journal_entry.set_amount_on_last_row(frm, cdt, cdn);
				haloerp.journal_entry.set_debit_credit_in_company_currency(frm, cdt, cdn);
	...
	set_amount_on_last_row(frm, cdt, cdn) {
		...
		haloerp.journal_entry.set_balancing_amount(row, difference);
	},
	...
	setup_queries(frm) {
		...
		frm.set_query("project", "accounts", (doc, cdt, cdn) => {
			...
			return { query: "haloerp.controllers.queries.get_project_name", filters };
		});

		frm.set_query("account", "accounts", () => haloerp.journal_entry.account_query(frm));

		frm.set_query("party_type", "accounts", (doc, cdt, cdn) => {
			return {
				query: "haloerp.setup.doctype.party_type.party_type.get_party_type",
				filters: { account: frappe.get_doc(cdt, cdn).account },
			};
		});

		frm.set_query("reference_name", "accounts", (doc, cdt, cdn) => {
			return haloerp.journal_entry.reference_name_query(frappe.get_doc(cdt, cdn));
		});
	},
	...
	reference_name_query(row) {
		if (row.reference_type === "Journal Entry") {
			...
			return {
				query: "haloerp.accounts.doctype.journal_entry.journal_entry.get_against_jv",
	...
	set_exchange_rate(frm, cdt, cdn) {
		...
		if (row.account_currency == company_currency || !frm.doc.multi_currency) {
			...
			frappe.call({
				method: "haloerp.accounts.doctype.journal_entry.journal_entry.get_exchange_rate",
				args: {
