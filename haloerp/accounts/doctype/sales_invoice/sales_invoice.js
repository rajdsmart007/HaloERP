...
haloerp.accounts.taxes.setup_tax_validations("Sales Invoice");
haloerp.accounts.payment_triggers.setup("Sales Invoice");
haloerp.accounts.pos.setup("Sales Invoice");
haloerp.accounts.taxes.setup_tax_filters("Sales Taxes and Charges");
haloerp.sales_common.setup_selling_controller();
haloerp.accounts.SalesInvoiceController = class SalesInvoiceController extends (
	haloerp.selling.SellingController
) {
	setup(doc) {
	...
	onload() {
		...
		haloerp.queries.setup_queries(this.frm, "Warehouse", function () {
			return haloerp.queries.warehouse(me.frm.doc);
		});
		...
		haloerp.queries.setup_warehouse_query(this.frm);
	}
	...
	refresh(doc, dt, dn) {
		...
		if (doc.docstatus == 1 && !doc.inter_company_invoice_reference) {
			...
			if (internal) {
				...
				frappe.call({
					method: "haloerp.accounts.doctype.sales_invoice.mapper.get_received_items",
					args: {
		...
		haloerp.accounts.unreconcile_payment.add_unreconcile_btn(me.frm);
	...
	make_invoice_discounting() {
		frappe.model.open_mapped_doc({
			method: "haloerp.accounts.doctype.sales_invoice.mapper.create_invoice_discounting",
			frm: this.frm,
		});
	}
	...
	make_dunning() {
		frappe.model.open_mapped_doc({
			method: "haloerp.accounts.doctype.sales_invoice.mapper.create_dunning",
			frm: this.frm,
		});
	}
	...
	make_maintenance_schedule() {
		frappe.model.open_mapped_doc({
			method: "haloerp.accounts.doctype.sales_invoice.mapper.make_maintenance_schedule",
			frm: this.frm,
		});
	}
				...
				let d = new frappe.ui.Dialog({
					...
					fields: [
						...
						},
						{
							...
							get_query: () => {
								return {
									query: "haloerp.controllers.queries.item_query",
									filters: {
										is_sales_item: 1,
		...
		this.$sales_order_btn = this.frm.add_custom_button(
			...
			function () {
				haloerp.utils.map_current_doc({
					method: "haloerp.selling.doctype.sales_order.mapper.make_sales_invoice",
	...
	quotation_btn() {
		...
		this.$quotation_btn = this.frm.add_custom_button(
			...
			function () {
				haloerp.utils.map_current_doc({
					method: "haloerp.selling.doctype.quotation.mapper.make_sales_invoice",
			...
			function () {
				...
				haloerp.utils.map_current_doc({
					...
					get_query: function () {
						...
						return {
							query: "haloerp.controllers.queries.get_delivery_notes_to_be_billed",
							filters: filters,
						};
					},
					allow_child_item_selection: true,
	...
	customer() {
		...
		haloerp.utils.get_party_details(
			this.frm,
			"haloerp.accounts.party.get_party_details",
			{
		...
		if (this.frm.doc.customer) {
			frappe.call({
				method: "haloerp.accounts.doctype.sales_invoice.sales_invoice.get_loyalty_programs",
				args: {
	...
	make_inter_company_invoice() {
		let me = this;
		frappe.model.open_mapped_doc({
			method: "haloerp.accounts.doctype.sales_invoice.mapper.make_inter_company_purchase_invoice",
			frm: me.frm,
		});
	}
	...
	make_sales_return() {
		frappe.model.open_mapped_doc({
			method: "haloerp.accounts.doctype.sales_invoice.mapper.make_sales_return",
			frm: this.frm,
		});
	}
	...
	asset(frm, cdt, cdn) {
		...
		if (row.asset) {
			frappe.call({
				method: haloerp.assets.doctype.asset.depreciation.get_disposal_account_and_cost_center,
				args: {
...
extend_cscript(cur_frm.cscript, new haloerp.accounts.SalesInvoiceController({ frm: cur_frm }));

cur_frm.cscript["Make Delivery Note"] = function () {
	frappe.model.open_mapped_doc({
		method: "haloerp.accounts.doctype.sales_invoice.mapper.make_delivery_note",
		frm: cur_frm,
	});
};

cur_frm.cscript.income_account = function (doc, cdt, cdn) {
	haloerp.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "income_account");
};

cur_frm.cscript.expense_account = function (doc, cdt, cdn) {
	haloerp.utils.copy_value_in_all_rows(doc, cdt, cdn, "items", "expense_account");
};
...
frappe.ui.form.on("Sales Invoice", {
	setup: function (frm) {
		...
		frm.set_query("income_account", "items", function (doc) {
			return {
				query: "haloerp.controllers.queries.get_income_account",
				filters: {
					company: doc.company,
					disabled: 0,
				},
		...
		frm.set_query("time_sheet", "timesheets", function (doc, cdt, cdn) {
			return {
				query: "haloerp.projects.doctype.timesheet.timesheet.get_timesheet",
		...
		frm.set_query("pos_profile", function (doc) {
			...
			return {
				query: "haloerp.accounts.doctype.pos_profile.pos_profile.pos_profile_query",
				filters: {
					company: doc.company,
				},
			};
	...
	loyalty_points: function (frm) {
		if (frm.redemption_conversion_factor) {
			...
			frappe.call({
				method: "haloerp.accounts.doctype.loyalty_program.loyalty_program.get_redeemption_factor",
				args: {
	...
	get_loyalty_details: function (frm) {
		if (frm.doc.customer && frm.doc.redeem_loyalty_points) {
			frappe.call({
				method: "haloerp.accounts.doctype.loyalty_program.loyalty_program.get_loyalty_program_details",
				args: {
	...
	async get_timesheet_data(frm, kwargs) {
		return frappe
			.call({
				method: "haloerp.projects.doctype.timesheet.timesheet.get_projectwise_timesheet_data",
				args: kwargs,
			})
			.then((r) => {
...
var set_timesheet_detail_rate = function (cdt, cdn, currency, timelog) {
	frappe.call({
		method: "haloerp.projects.doctype.timesheet.timesheet.get_timesheet_detail_rate",
		args: {
			timelog: timelog,
			currency: currency,
		},
