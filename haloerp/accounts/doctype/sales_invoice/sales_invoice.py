...
import haloerp
from haloerp.accounts.deferred_revenue import validate_service_stop_date
from haloerp.accounts.doctype.loyalty_program.loyalty_program import validate_loyalty_points
from haloerp.accounts.doctype.pricing_rule.utils import (
	update_coupon_code_count,
	validate_coupon_code,
)
from haloerp.accounts.doctype.repost_accounting_ledger.repost_accounting_ledger import (
	validate_docs_for_deferred_accounting,
	validate_docs_for_voucher_types,
)
from haloerp.accounts.doctype.tax_withholding_entry.tax_withholding_entry import SalesTaxWithholding
from haloerp.accounts.party import get_due_date, get_party_account
from haloerp.accounts.utils import refresh_subscription_status, update_voucher_outstanding
from haloerp.controllers.accounts_controller import validate_account_head
from haloerp.controllers.selling_controller import SellingController
from haloerp.setup.doctype.company.company import update_company_current_month_sales
from haloerp.stock.doctype.delivery_note.services.billing_status import (
	update_billed_amount_based_on_so,
)
from haloerp.stock.utils import get_bin_qty_map
...
class SalesInvoice(SellingController):
	...
	if TYPE_CHECKING:
		...
		from haloerp.accounts.doctype.item_wise_tax_detail.item_wise_tax_detail import ItemWiseTaxDetail
		from haloerp.accounts.doctype.payment_schedule.payment_schedule import PaymentSchedule
		from haloerp.accounts.doctype.pricing_rule_detail.pricing_rule_detail import PricingRuleDetail
		from haloerp.accounts.doctype.sales_invoice_advance.sales_invoice_advance import SalesInvoiceAdvance
		from haloerp.accounts.doctype.sales_invoice_item.sales_invoice_item import SalesInvoiceItem
		from haloerp.accounts.doctype.sales_invoice_payment.sales_invoice_payment import SalesInvoicePayment
		from haloerp.accounts.doctype.sales_invoice_timesheet.sales_invoice_timesheet import (
			SalesInvoiceTimesheet,
		)
		from haloerp.accounts.doctype.sales_taxes_and_charges.sales_taxes_and_charges import (
			SalesTaxesandCharges,
		)
		from haloerp.accounts.doctype.tax_withholding_entry.tax_withholding_entry import TaxWithholdingEntry
		from haloerp.selling.doctype.sales_team.sales_team import SalesTeam
		from haloerp.stock.doctype.packed_item.packed_item import PackedItem
	...
	def validate(self):
		...
		from haloerp.accounts.services.billing_validation import BillingValidationService
	...
	def check_credit_limit(self):
		from haloerp.selling.doctype.customer.customer import check_credit_limit
	...
	def check_overdue_billing_threshold(self):
		from haloerp.selling.doctype.customer.customer import check_overdue_billing_threshold
	...
	def update_packing_list(self):
		if cint(self.update_stock) == 1:
			from haloerp.stock.doctype.packed_item.packed_item import make_packing_list
	...
	def make_gl_entries(self, gl_entries=None, from_repost=False):
		from haloerp.accounts.general_ledger import make_gl_entries, make_reverse_gl_entries

		auto_accounting_for_stock = haloerp.is_perpetual_inventory_enabled(self.company)
		if not gl_entries:
...
def get_list_context(context=None):
	from haloerp.controllers.website_list_for_contact import get_list_context
