...
from haloerp.accounts.services.base_gl_composer import BaseGLComposer
from haloerp.accounts.services.taxes import TaxService
from haloerp.accounts.utils import get_account_currency
...
class SalesInvoiceGLComposer(BaseGLComposer):
	...
	def compose(self, inventory_account_map=None):
		from haloerp.accounts.doctype.sales_invoice.sales_invoice import make_regional_gl_entries
		from haloerp.accounts.general_ledger import merge_similar_entries
				...
				doc.get_gl_dict(
					{
						...
						"cost_center": doc.cost_center or haloerp.get_default_cost_center(doc.company),
					},
					item=doc,
				)
			)
	...
	def stock_delivered_but_not_billed_gl_entries(self, gl_entries):
		doc = self.doc
		if doc.update_stock or not cint(haloerp.is_perpetual_inventory_enabled(doc.company)):
			return
	...
	def make_item_gl_entries(self, gl_entries):
		from haloerp.accounts.doctype.sales_invoice.sales_invoice import SalesInvoice
		...
		if cint(doc.update_stock) and haloerp.is_perpetual_inventory_enabled(doc.company):
