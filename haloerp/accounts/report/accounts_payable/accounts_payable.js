...
function create_payment_entries_from_payable_report(report) {
	...
	frappe.call({
		method: "haloerp.accounts.bulk_payment.get_payable_invoices",
...
function show_create_payment_entries_dialog(report, payable, excluded, currency) {
	...
	const dialog = new frappe.ui.Dialog({
		...
		primary_action() {
			...
			frappe
				.call({
					method: "haloerp.accounts.bulk_payment.create_payment_entries",
					args: { invoices },
				})
				.then(clearSelection)
...
haloerp.utils.add_dimensions("Accounts Payable", 10);
