...
haloerp.accounts.bank_reconciliation.NumberCardManager = class NumberCardManager {
	constructor(opts) {
	...
	make_cards() {
		...
		chart_data.forEach((summary) => {
			let number_card = new haloerp.accounts.NumberCard(summary);
...
haloerp.accounts.NumberCard = class NumberCard {
	constructor(options) {
