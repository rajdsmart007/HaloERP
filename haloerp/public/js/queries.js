...
frappe.provide("haloerp.queries");
$.extend(haloerp.queries, {
	user: function () {
	...
	party: function (doc) {
		return {
			query: "haloerp.controllers.queries.party_query",
			filters: { disabled: 0, company: doc.company },
		};
	},

	customer: function (doc) {
		return haloerp.queries.party(doc);
	},
	...
	item: function (filters) {
		var args = { query: "haloerp.controllers.queries.item_query" };
		if (filters) args["filters"] = filters;
		return args;
	},

	bom: function () {
		return { query: "haloerp.controllers.queries.bom" };
	},

	task: function () {
		return { query: "haloerp.projects.utils.query_task" };
	},
	...
	employee: function () {
		return { query: "haloerp.controllers.queries.employee_query" };
	},
...
haloerp.queries.setup_queries = function (frm, options, query_fn) {
	var me = this;
...
*/
haloerp.queries.setup_warehouse_query = function (frm) {
	frm.set_query("warehouse", "items", function (doc, cdt, cdn) {
		var row = locals[cdt][cdn];
		var filters = haloerp.queries.warehouse(frm.doc);
		if (row.item_code) {
			$.extend(filters, { query: "haloerp.controllers.queries.warehouse_query" });
