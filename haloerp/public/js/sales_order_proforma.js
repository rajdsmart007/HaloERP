...
frappe.ui.form.on("Sales Order", {
	refresh(frm) {
		haloerp.proforma.toggle_tab(frm, false);
		...
		frappe.db.get_single_value("Selling Settings", "enable_proforma_invoice").then((enabled) => {
			...
			setTimeout(() => {
				frm.add_custom_button(
					__("Proforma Invoice"),
					() => haloerp.proforma.open_dialog(frm),
					__("Create")
				);
			}, 0);
			haloerp.proforma.render_list(frm);
		});
	},
});

frappe.provide("haloerp.proforma");

Object.assign(haloerp.proforma, {
	toggle_tab(frm, show) {
	...
	open_dialog(frm) {
		frappe.call({
			method: "haloerp.selling.doctype.proforma_invoice.proforma_invoice.get_sales_order_items",
					...
					fields: [
						...
						},
						{
							...
							onchange: function () {
								...
								haloerp.proforma.update_warning(dialog);
							},
						},
						{
							fieldname: "amount",
	...
	create(frm, dialog, values) {
		...
		frappe.call({
			method: "haloerp.selling.doctype.proforma_invoice.proforma_invoice.make_proforma_invoice",
			args: {
	...
	build_list(frm) {
		...
		const list = new frappe.ui.EmbeddedList({
			...
			after_render() {
				...
				haloerp.proforma.toggle_tab(frm, has_proformas);
	...
	send_email(frm, proforma_name, refresh) {
		frappe.prompt(
			...
			(values) => {
				frappe.call({
					method: "haloerp.selling.doctype.proforma_invoice.proforma_invoice.send_proforma_email",
