...
frappe.setup.on("before_load", function () {
	...
	haloerp.setup.slides_settings.map(frappe.setup.add_slide);
});

haloerp.setup.slides_settings = [
	{
		...
		apply_industry_modules: function (slide) {
			...
			let modules = haloerp.setup.industry_modules[industry] || ["accounting"];
	...
	},
	{
		...
		set_fy_dates: function (slide) {
			...
			if (country) {
				let fy = haloerp.setup.fiscal_years[country];
		...
		charts_modal: function (slide, chart_template) {
			...
			let coa_tree = new frappe.ui.Tree({
				...
				label: parent,
				expandable: true,
				method: "haloerp.accounts.utils.get_coa",
				args: {
...
haloerp.setup.industry_modules = {
