...
class ShopFloor {
	...
	init() {
		frappe.call("haloerp.manufacturing.page.shop_floor.shop_floor.get_shop_floor_context").then((r) => {
			const ctx = r.message || {};
	...
	make() {
		...
		this.brand_icon = `<img class="sf-brand-icon" src="/assets/haloerp/images/haloerp-logo.svg" alt="${__(
			"HaloERP"
		)}">`;
	...
	load_bucket(bucket, append = false) {
		...
		frappe.call({
			method: "haloerp.manufacturing.page.shop_floor.shop_floor.get_work_orders",
			args: {
				status_group: bucket,
				start: start,
	...
	load_operator_data($container, { workstation, work_order }) {
		frappe.call({
			method: "haloerp.manufacturing.page.shop_floor.shop_floor.get_data",
			args: {
	...
	end_session(jc_name) {
		...
		const save_and_continue = () => {
			...
			frappe.call({
				method: "haloerp.manufacturing.page.shop_floor.shop_floor.save_and_continue",
				args: args,
				freeze: true,
		...
		const finalize_submit = (args) => {
			frappe.call({
				method: "haloerp.manufacturing.page.shop_floor.shop_floor.complete_and_submit",
				args: args,
				freeze: true,
	...
	run_quality_check(jc, on_pass) {
		const me = this;
		frappe.call({
			method: "haloerp.manufacturing.page.shop_floor.shop_floor.get_quality_inspection_checklist",
	...
	show_qc_dialog(jc, info, on_pass) {
		...
		const dialog = new frappe.ui.Dialog({
			...
			primary_action: () => {
				...
				dialog.hide();
				frappe.call({
					method: "haloerp.manufacturing.page.shop_floor.shop_floor.submit_quality_inspection",
	...
	submit_job_card(jc_name) {
		...
		frappe.confirm(__("Submit job card {0}? This finalizes the job card.", [jc_name]), () => {
			frappe.call({
				method: "haloerp.manufacturing.page.shop_floor.shop_floor.submit_job_card",
	...
	make_manufacture_entry(jc_name) {
		frappe.call({
			method: "haloerp.manufacturing.page.shop_floor.shop_floor.make_manufacture_stock_entry",
			args: { job_card: jc_name },
			freeze: true,
	...
	transfer_materials(jc_name) {
		if (!jc_name) return;
		frappe.call({
			method: "haloerp.manufacturing.doctype.job_card.job_card.make_stock_entry",
	...
	update_job_card(job_card, method, data, on_success) {
		const me = this;
		frappe.call({
			method: "haloerp.manufacturing.doctype.workstation.workstation.update_job_card",
			args: {
				job_card: job_card,
				method: method,
