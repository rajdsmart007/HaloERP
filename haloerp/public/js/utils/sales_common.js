...
frappe.provide("haloerp.selling");

haloerp.sales_common = {
	setup_selling_controller: function () {
		haloerp.selling.SellingController = class SellingController extends haloerp.TransactionController {
			setup() {
				super.setup();
			...
			setup_queries() {
				...
				$.each(
					...
					function (i, opts) {
						if (me.frm.fields_dict[opts[0]]) me.frm.set_query(opts[0], haloerp.queries[opts[1]]);
					}
				);

				me.frm.set_query("contact_person", haloerp.queries.contact_query);
				me.frm.set_query("shipping_contact_person", haloerp.queries.contact_query);
				me.frm.set_query("company_contact_person", haloerp.queries.company_contact_query);
				me.frm.set_query("customer_address", haloerp.queries.address_query);
				me.frm.set_query("shipping_address_name", haloerp.queries.address_query);
				me.frm.set_query("dispatch_address_name", haloerp.queries.dispatch_address_query);
				me.frm.set_query("company_address", haloerp.queries.company_address_query);
			...
			set_default_company_address() {
				...
				if (this.frm.doc.company) {
					frappe.call({
						method: "haloerp.setup.doctype.company.company.get_default_company_address",
						args: {
			...
			customer_address() {
				haloerp.utils.get_address_display(this.frm, "customer_address");
			...
			shipping_address_name() {
				haloerp.utils.get_address_display(this.frm, "shipping_address_name", "shipping_address");
				haloerp.utils.set_taxes_from_address(
					this.frm,
					"shipping_address_name",
					"customer_address",
			...
			dispatch_address_name() {
				haloerp.utils.get_address_display(this.frm, "dispatch_address_name", "dispatch_address");
			}
			...
			project(doc, cdt, cdn) {
				...
				if (["Delivery Note", "Sales Invoice", "Sales Order"].includes(this.frm.doc.doctype)) {
					if (this.frm.doc.project) {
						frappe.call({
							method: "haloerp.projects.doctype.project.project.get_cost_center_name",
