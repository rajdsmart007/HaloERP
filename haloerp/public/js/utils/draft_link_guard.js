...
haloerp.utils.confirm_if_drafts_exist = async function (source_doc, target_doctype) {
	...
	let drafts;
	try {
		drafts = await frappe.xcall("haloerp.controllers.draft_links.get_existing_drafts", {
...
if (frappe.model.add_mapped_doc_guard) {
	frappe.model.add_mapped_doc_guard((mapped_doc, opts) =>
		haloerp.utils.confirm_if_drafts_exist(opts.frm && opts.frm.doc, mapped_doc.doctype)
	);
}
