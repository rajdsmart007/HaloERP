...
haloerp.stock.SerialBatchInlineEditor = class SerialBatchInlineEditor {
	constructor({ frm, cdt, cdn, wrapper, is_rejected }) {
		...
		this.config = haloerp.stock.get_sbie_config(frm.doc.doctype, cdt) || {};
	...
	get pending() {
		let store = haloerp.stock.get_sbie_pending_map(this.frm);
	...
	clear_pending() {
		delete haloerp.stock.get_sbie_pending_map(this.frm)[this.pending_key];
	}
	...
	download_csv() {
		let url;
		if (this.bundle) {
			url = `/api/method/haloerp.stock.doctype.serial_and_batch_bundle.inline_editor.download_bundle_entries_csv?bundle=${encodeURIComponent(
				this.bundle
			)}`;
		} else {
			url = `/api/method/haloerp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.download_blank_csv_template?content=${encodeURIComponent(
	...
	async import_csv_file(file_url) {
		let data = await this.call(
			"haloerp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.upload_csv_file",
	...
	async open_auto_fetch_dialog() {
		...
		let based_on = await haloerp.stock.get_pick_serial_batch_based_on();
	...
	async auto_fetch_entries(qty, based_on, warehouse) {
		let data = await this.call(
			"haloerp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle.get_auto_data",
			{
	...
	open_range_dialog() {
		...
		let dialog = new frappe.ui.Dialog({
			...
			primary_action: ({ serial_no_range }) => {
				let serial_nos = haloerp.stock.utils.get_serial_range(serial_no_range, "::");
	...
	async load_page() {
		if (!this.bundle) {
			...
			let data = await this.call(
				"haloerp.stock.doctype.serial_and_batch_bundle.inline_editor.get_bundle_entries",
				{
					bundle: this.bundle,
					start: this.start,
	...
	async upsert({ entries = [], deleted = [], replace = 0 }) {
		let summary = await this.call(
			"haloerp.stock.doctype.serial_and_batch_bundle.inline_editor.upsert_bundle_entries",
			{
...
haloerp.stock.get_sbie_config = function (doctype, child_doctype) {
	return haloerp.stock.SBIE_DOCTYPES.find((d) => d.parent === doctype && d.child === child_doctype);
};

haloerp.stock.get_sbie_row = function (frm, cdn) {
	for (let config of haloerp.stock.SBIE_DOCTYPES) {
...
haloerp.stock.get_sbie_pending_map = function (frm) {
	let store = (frm._sbie_pending = frm._sbie_pending || {});
	return (store[frm.doc.name] = store[frm.doc.name] || {});
};

haloerp.stock.flush_serial_batch_pending = async function (frm) {
	let pending_map = haloerp.stock.get_sbie_pending_map(frm);
	...
	for (let key of Object.keys(pending_map)) {
		...
		let { row, config } = haloerp.stock.get_sbie_row(frm, cdn);
		if (!row) {
			delete pending_map[key];
			continue;
		}
...
haloerp.stock.mount_serial_batch_inline_editor = async function (frm, cdt, cdn) {
	let config = haloerp.stock.get_sbie_config(frm.doc.doctype, cdt);
	...
	let enabled = await haloerp.stock.is_inline_serial_batch_editor_enabled();
	let row = locals[cdt][cdn];
	let show = enabled && row && !row.use_serial_batch_fields && frm.doc.docstatus === 0;

	haloerp.stock.toggle_legacy_bundle_fields(grid_form, show);
	...
	for (let editor of editors) {
		...
		editors_store[key] = new haloerp.stock.SerialBatchInlineEditor({
			frm,
			cdt,
			cdn,
...
haloerp.stock.toggle_legacy_bundle_fields = function (grid_form, editor_active) {
	let legacy_fields = [
...
haloerp.stock.setup_serial_batch_pending_flush = function (doctype) {
	frappe.ui.form.on(doctype, {
		validate(frm) {
			return haloerp.stock.flush_serial_batch_pending(frm);
		},
	});
};

haloerp.stock.setup_inline_serial_batch_editor = function () {
	new Set(haloerp.stock.SBIE_DOCTYPES.map((d) => d.parent)).forEach((doctype) =>
		haloerp.stock.setup_serial_batch_pending_flush(doctype)
	);

	new Set(haloerp.stock.SBIE_DOCTYPES.map((d) => d.child)).forEach((child_doctype) => {
		frappe.ui.form.on(child_doctype, {
			form_render(frm, cdt, cdn) {
				haloerp.stock.mount_serial_batch_inline_editor(frm, cdt, cdn);
			},
...
haloerp.stock.setup_inline_serial_batch_editor();

haloerp.stock.is_inline_serial_batch_editor_enabled = async function () {
	if (haloerp.stock._inline_editor_enabled === undefined) {
		let { message } = await frappe.db.get_value(
			"Stock Settings",
			"Stock Settings",
			"use_inline_serial_batch_editor"
		);
		haloerp.stock._inline_editor_enabled = cint(message && message.use_inline_serial_batch_editor);
	}

	return haloerp.stock._inline_editor_enabled;
};

haloerp.stock.get_pick_serial_batch_based_on = async function () {
	if (haloerp.stock._pick_serial_batch_based_on === undefined) {
		let { message } = await frappe.db.get_value(
			"Stock Settings",
			"Stock Settings",
			"pick_serial_and_batch_based_on"
		);
		haloerp.stock._pick_serial_batch_based_on =
			(message && message.pick_serial_and_batch_based_on) || "FIFO";
	}

	return haloerp.stock._pick_serial_batch_based_on;
};
