...
from haloerp.stock.doctype.serial_and_batch_bundle.serial_and_batch_bundle import get_available_serial_nos
...
from haloerp.stock.valuation import FIFOValuation, LIFOValuation
...
def get_incoming_rate(args: dict | str, raise_error_if_no_rate: bool = True, fallbacks: bool = True):
	...
	if in_rate is None:
		...
		in_rate = get_valuation_rate(
			...
			currency=haloerp.get_company_currency(args.get("company")),
	...
	if reposting_pending and throw_error:
		...
		frappe.msgprint(
			...
			primary_action={
				...
				"client_action": "haloerp.route_to_pending_reposts",
				"args": filters,
			},
		)
...
def _update_item_info(scan_result: dict[str, str | None], ctx: dict | None = None) -> dict[str, str | None]:
	from haloerp.stock.get_item_details import get_item_warehouse_
