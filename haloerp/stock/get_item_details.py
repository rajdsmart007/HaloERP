...
import haloerp
from haloerp import get_company_currency
from haloerp.accounts.doctype.pricing_rule.pricing_rule import (
...
from haloerp.stock.doctype.item.item import get_item_defaults, get_uom_conv_factor
from haloerp.stock.doctype.item_manufacturer.item_manufacturer import get_item_manufacturer_part_no
from haloerp.stock.doctype.price_list.price_list import get_price_list_details
...
@frappe.whitelist()
@haloerp.normalize_ctx_input(ItemDetailsCtx)
...
def update_stock(ctx, out, doc=None):
	from haloerp.stock.doctype.batch.batch import get_available_batches
	from haloerp.stock.doctype.serial_no.serial_no import get_serial_nos_for_outward
...
def validate_item_details(ctx: frappe._dict, item):
	...
	from haloerp.stock.doctype.item.item import validate_end_of_life
...
def get_basic_details(ctx: frappe._dict, item, overwrite_warehouse=True) -> frappe._dict:
	...
	if item.is_fixed_asset:
		from haloerp.assets.doctype.asset.asset import get_asset_account, is_cwip_accounting_enabled
		...
		if is_cwip_accounting_enabled(item.asset_category):
			...
			from haloerp.assets.doctype.asset_category.asset_category import get_asset_category_account
	...
	if ctx.doctype in purchase_doctypes and not frappe.db.get_single_value(
		...
		from haloerp.buying.doctype.purchase_order.purchase_order import item_last_purchase_rate
	...
	if (
		item.is_stock_item
		and haloerp.is_perpetual_inventory_enabled(ctx.company)
		and (
...
def get_default_cost_center(ctx: ItemDetailsCtx, item=None, item_group=None, brand=None, company=None):
	...
	if not cost_center and (item and item_group and brand):
		...
		for method in ["get_item_defaults", "get_item_group_defaults", "get_brand_defaults"]:
			path = f"haloerp.stock.get_item_details.{method}"
...
def get_price_list_rate(ctx: frappe._dict, item_doc, out: frappe._dict = None):
	...
	if meta.get_field("currency") or ctx.get("currency"):
		...
		if not ctx.is_internal_supplier and not out.price_list_rate and ctx.transaction_type == "buying":
			from haloerp.stock.doctype.item.item import get_last_purchase_details
...
def validate_conversion_rate(ctx: frappe._dict, meta):
	from haloerp.controllers.accounts_controller import validate_conversion_rate
...
def get_bin_details(
	...
	if warehouse:
		...
		from haloerp.stock.doctype.warehouse.warehouse import get_child_warehouses
...
def get_batch_qty(batch_no: str, warehouse: str, item_code: str):
	from haloerp.stock.doctype.batch import batch
