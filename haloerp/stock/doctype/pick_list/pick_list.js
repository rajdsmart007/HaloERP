...
frappe.ui.form.on("Pick List", {
	...
	setup: (frm) => {
		...
		frm.set_query("work_order", () => {
			return {
				query: "haloerp.stock.doctype.pick_list.pick_list.get_pending_work_orders",
				filters: {
				...
				frappe.prompt(
					...
					(data) => {
						...
						haloerp.utils.map_current_doc({
							method: "haloerp.manufacturing.doctype.work_order.mapper.create_pick_list",
							target: frm,
	...
	create_delivery(frm, doctype) {
		frappe.model.open_mapped_doc({
			method: "haloerp.stock.doctype.pick_list.mapper.create_delivery",
			args: {
				target: doctype,
			},
			frm: frm,
	...
	create_stock_entry: (frm) => {
		frappe
			.xcall("haloerp.stock.doctype.pick_list.mapper.create_stock_entry", {
				pick_list: frm.doc,
			})
			.then((stock_entry) => {
	...
	scan_barcode: (frm) => {
		...
		const barcode_scanner = new haloerp.utils.BarcodeScanner(opts);
	...
	show_stock_availability(frm) {
		...
		frappe
			.xcall("haloerp.stock.doctype.pick_list.pick_list.get_stock_availability", {
				items: items,
...
function get_item_details(item_code, uom = null, warehouse = null, company = null) {
	if (item_code) {
		return frappe.xcall("haloerp.stock.doctype.pick_list.pick_list.get_item_details", {
			item_code,
			uom,
			warehouse,
			company,
