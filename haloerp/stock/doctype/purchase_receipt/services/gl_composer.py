...
from haloerp.stock import get_warehouse_account
...
class PurchaseReceiptGLComposer(BaseStockGLComposer):
	...
	def compose(
		...
		from haloerp.stock.doctype.purchase_receipt.purchase_receipt import update_regional_gl_entries
	...
	def _make_item_gl_entries(self, gl_entries: list, inventory_account_map: dict | None) -> None:
		from haloerp.accounts.doctype.purchase_invoice.purchase_invoice import (
			get_purchase_document_details,
		)
		from haloerp.stock.doctype.purchase_receipt.purchase_receipt import get_stock_value_difference
		...
		for d in doc.get("items"):
			...
			if (
				...
				if not (
					(haloerp.is_perpetual_inventory_enabled(doc.company) and d.item_code in stock_items)
