...
import haloerp
from haloerp.controllers.item_variant import (
...
from haloerp.stock.doctype.item_default.item_default import ItemDefault
...
class Item(Document):
	...
	if TYPE_CHECKING:
		...
		from haloerp.stock.doctype.company_restriction.company_restriction import CompanyRestriction
		from haloerp.stock.doctype.item_barcode.item_barcode import ItemBarcode
		from haloerp.stock.doctype.item_customer_detail.item_customer_detail import ItemCustomerDetail
		from haloerp.stock.doctype.item_default.item_default import ItemDefault
		from haloerp.stock.doctype.item_reorder.item_reorder import ItemReorder
		from haloerp.stock.doctype.item_supplier.item_supplier import ItemSupplier
		from haloerp.stock.doctype.item_tax.item_tax import ItemTax
		from haloerp.stock.doctype.item_variant_attribute.item_variant_attribute import ItemVariantAttribute
		from haloerp.stock.doctype.uom_conversion_detail.uom_conversion_detail import UOMConversionDetail
			...
			item_price = frappe.get_doc(
				{
					...
					"uom": self.stock_uom,
					"brand": self.brand,
					"currency": haloerp.get_default_currency(),
	...
	def recalculate_bin_qty(self, new_name):
		from haloerp.stock.stock_balance import repost_stock
		...
		if self.has_variants:
			...
			if variants:
				if len(variants) <= 30:
					...
					frappe.enqueue(
						"haloerp.stock.doctype.item.item.update_variants",
						variants=variants,
						template=self,
						now=frappe.in_test,
						timeout=600,
...
def get_asset_naming_series():
	from haloerp.assets.doctype.asset.asset import get_asset_naming_series
...
def on_doctype_update():
	if frappe.db.db_type == "postgres":
		# The Item link-search (haloerp.controllers.queries.item_query) filters
