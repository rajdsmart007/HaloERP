...
frappe.ui.form.on("Item", {
	...
	allow_negative_stock(frm) {
		haloerp.utils.confirm_negative_stock(frm);
	},
	...
	setup: function (frm) {
		frm.set_query("allowed_companies", () => ({
			query: "haloerp.stock.doctype.company_restriction.company_restriction.company_query",
		}));
	...
	onload: function (frm) {
		haloerp.item.setup_queries(frm);
	...
	refresh: function (frm) {
		...
		if (frm.doc.is_stock_item) {
			...
			if (can_create_stock_entry && !has_existing_stock) {
				frm.add_custom_button(
					...
					() => haloerp.item.show_opening_stock_dialog(frm),
					__("Actions")
				);
			}
		}
		...
		if (frm.doc.has_variants) {
			...
			if (frm.doc.variant_based_on === "Item Attribute") {
				frm.add_custom_button(
					__("Single Variant"),
					function () {
						haloerp.item.show_single_variant_dialog(frm);
					},
					__("Create")
				);
				frm.add_custom_button(
					...
					function () {
						haloerp.item.show_multiple_variants_dialog(frm);
					},
					__("Create")
				);
			} else {
				frm.add_custom_button(
					__("Variant"),
					function () {
						haloerp.item.show_modal_for_manufacturers(frm);
					},
					__("Create")
				);
			}
		...
		haloerp.item.toggle_attributes(frm);

		if (!frm.doc.is_fixed_asset) {
			haloerp.item.make_dashboard(frm);
		}

		haloerp.item.render_item_prices(frm);
	...
	validate: function (frm) {
		haloerp.item.weight_to_validate(frm);
	},
	...
	is_fixed_asset: function (frm) {
		...
		frappe.call({
			method: "haloerp.stock.doctype.item.item.get_asset_naming_series",
			callback: function (r) {
...
frappe.ui.form.on("Item Default", {
	form_render: function (frm, cdt, cdn) {
		...
		haloerp.item.populate_virtual_fields(frm, cdt, cdn, row);
	},
...
$.extend(haloerp.item, {
		...
		frappe.call({
			...
			callback: function (r) {
				...
				if (!needs_company_fallback.length) {
					setTimeout(() => haloerp.item.update_vf_labels(frm, cdn, {}), 50);
					return;
				}
				frappe.call({
					method: "haloerp.setup.doctype.item_group.item_group.get_company_resolved_defaults",
					...
					callback: function (cr) {
						...
						setTimeout(() => haloerp.item.update_vf_labels(frm, cdn, from_company), 50);
					},
				});
			},
		});
	...
	setup_queries: function (frm) {
		...
		frm.fields_dict.reorder_levels.grid.get_field("warehouse_group").get_query = function (
			...
			let row = locals[cdt][cdn];
			return {
				query: "haloerp.stock.doctype.warehouse.warehouse.get_warehouses_for_reorder",
				filters: {
	...
	make_dashboard: function (frm) {
		...
		if (frm.doc.is_stock_item) {
			frappe.require("item-dashboard.bundle.js", function () {
				...
				haloerp.item.item_dashboard = new haloerp.stock.ItemDashboard({
					parent: section,
					item_code: frm.doc.name,
					page_length: 20,
					method: "haloerp.stock.dashboard.item_dashboard.get_data",
					template: "item_dashboard_list",
				});
				haloerp.item.item_dashboard.refresh();
			});
		}
	},
	...
	render_item_prices: function (frm) {
		...
		frappe.require("embedded_list.bundle.js", () => haloerp.item.build_prices_list(frm));
	},
	...
	build_prices_list: function (frm) {
		...
		const list = new frappe.ui.EmbeddedList({
			...
			add_button: {
				label: __("Add Price"),
				action: () => haloerp.item.new_item_price(frm),
			},
			on_row_click: (row) => frappe.set_route("Form", "Item Price", row.name),
			get_data() {
				return frappe
					.xcall("haloerp.stock.doctype.item.item.get_item_prices", { item_code })
					.then((r) => {
	...
	show_opening_stock_dialog: function (frm) {
		...
		const dialog = new frappe.ui.Dialog({
			...
			primary_action: function (values) {
				frappe.call({
					method: "haloerp.stock.doctype.item.item.make_opening_stock_entry",
					args: {
						item_code: frm.doc.name,
	...
	show_modal_for_manufacturers: function (frm) {
		...
		dialog.set_primary_action(__("Create"), function () {
			...
			frappe.call({
				method: "haloerp.controllers.item_variant.get_variant",
				args: data,
				callback: function (r) {
	...
	show_multiple_variants_dialog: function (frm) {
		...
		function make_and_show_dialog(fields) {
			...
			me.multiple_variant_dialog.set_primary_action(__("Create Variants"), () => {
				...
				frappe.call({
					method: "haloerp.controllers.item_variant.enqueue_multiple_variant_creation",
					args: {
						item: frm.doc.name,
			...
			frappe.call({
				...
				callback: function (r) {
					...
					if (r.message) {
						...
						d.hide();
						frappe.call({
							method: "haloerp.controllers.item_variant.create_variant",
							args: {
								item: frm.doc.name,
								args: d.get_values(),
		...
		$.each(d.fields_dict, function (i, field) {
			...
			field.$input
				.on("input", function (e) {
					...
					frappe.call({
						method: "haloerp.stock.doctype.item.item.get_item_attribute",
						args: {
							parent: i,
...
frappe.ui.form.on("UOM Conversion Detail", {
	uom: function (frm, cdt, cdn) {
		...
		if (row.uom) {
			frappe.call({
				method: "haloerp.stock.doctype.item.item.get_uom_conv_factor",
				args: {
					uom: row.uom,
