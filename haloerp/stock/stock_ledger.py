...
import haloerp
from haloerp.stock.doctype.bin.bin import update_qty_from_sle
...
from haloerp.stock.valuation import FIFOValuation, LIFOValuation, round_off_if_near_zero
...
def validate_stock_frozen_by_closing_entry(sl_entries):
	from haloerp.stock.doctype.stock_closing_entry.stock_closing_entry import (
...
def make_sl_entries(sl_entries, allow_negative_stock=False, via_landed_cost_voucher=False):
	...
	args:
	        ...
	        such cases certain validations need to be ignored (like negative
	                        stock)
	"""
	from haloerp.controllers.stock_controller import future_sle_exists, invalidate_future_sle_cache
...
class update_entries_after:
	...
	def build(self):
		from haloerp.controllers.stock_controller import future_sle_exists
	...
	def process_sle(self, sle):
		...
		if not cint(haloerp.is_perpetual_inventory_enabled(sle.company)):
			return
	...
	def update_serial_batch_no_valuation(self, sle, doc, prev_sle=None):
		from haloerp.stock.serial_batch_bundle import BatchNoValuation, SerialNoValuation
	...
	def get_moving_average_rate_for_return(self, sle):
		...
		if rate is None:
			rate = get_valuation_rate(
				...
				currency=haloerp.get_company_currency(sle.company),
				company=sle.company,
			)
	...
	def update_batched_values(self, sle):
		from haloerp.stock.serial_batch_bundle import BatchNoValuation
...
def get_valuation_rate(
	...
	if (
		...
		and cint(haloerp.is_perpetual_inventory_enabled(company))
	):
