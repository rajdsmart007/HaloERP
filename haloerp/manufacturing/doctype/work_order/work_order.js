...
frappe.ui.form.on("Work Order", {
	setup: function (frm) {
		...
		frm.set_query("bom_no", function () {
			if (frm.doc.production_item) {
				return {
					query: "haloerp.controllers.queries.bom",
		...
		frm.set_query("operation", "required_items", function () {
			return {
				query: "haloerp.manufacturing.doctype.work_order.work_order.get_bom_operations",
				filters: {
		...
		frm.set_query("sales_order", function () {
			if (frm.doc.production_item) {
				return {
					query: "haloerp.manufacturing.doctype.work_order.work_order.query_sales_order",
					filters: {
	...
	onload: function (frm) {
		...
		if (frm.doc.__islocal) {
			...
			haloerp.work_order.set_default_warehouse(frm);
		}

		if (frm.doc.docstatus == 0 && frm.doc.bom_no) {
			frappe.call({
				method: "haloerp.manufacturing.doctype.work_order.work_order.check_if_scrap_warehouse_mandatory",
				args: {
	...
	refresh: function (frm) {
		...
		haloerp.work_order.set_custom_buttons(frm);
		frm.set_intro("");
		...
		haloerp.work_order.add_start_button(frm);
	...
	create_stock_return_entry: function (frm) {
		frappe.call({
			method: "haloerp.manufacturing.doctype.work_order.mapper.make_stock_return_entry",
			args: {
	...
	make_job_card: function (frm) {
		...
		const dialog = frappe.prompt(
			...
			function () {
				...
				frappe.call({
					method: "haloerp.manufacturing.doctype.work_order.work_order.make_job_card",
					freeze: true,
					args: {
						work_order: frm.doc.name,
	...
	make_disassembly_order(frm) {
		haloerp.work_order
			...
			.then((data) => {
				...
				return frappe.xcall("haloerp.manufacturing.doctype.work_order.mapper.make_stock_entry", {
	...
	production_item: function (frm) {
		if (frm.doc.production_item) {
			frappe.call({
				method: "haloerp.manufacturing.doctype.work_order.work_order.get_item_details",
				args: {
				...
				callback: function (r) {
					if (r.message) {
						...
						haloerp.in_production_item_onchange = true;
						...
						haloerp.in_production_item_onchange = false;
					}
				},
			});
		}
	...
	project: function (frm) {
		if (!haloerp.in_production_item_onchange && !frm.doc.bom_no) {
	...
	additional_operating_cost: function (frm) {
		haloerp.work_order.calculate_cost(frm.doc);
		haloerp.work_order.calculate_total_cost(frm);
	},
...
frappe.ui.form.on("Work Order Item", {
	...
	source_warehouse: function (frm, cdt, cdn) {
		...
		if (!row.item_code) {
			...
			frappe.call({
				method: "haloerp.stock.utils.get_latest_stock_qty",
				args: {
	...
	item_code: function (frm, cdt, cdn) {
		...
		if (row.item_code) {
			frappe.call({
				method: "haloerp.stock.doctype.item.item.get_item_details",
				args: {
					item_code: row.item_code,
					company: frm.doc.company,
				},
...
haloerp.work_order = {
		...
		if (doc.docstatus === 1 && !["Closed", "Completed"].includes(doc.status)) {
			frm.add_custom_button(
				...
				function () {
					frappe.confirm(__("Once the Work Order is Closed, it cannot be resumed."), () => {
						haloerp.work_order.change_work_order_status(frm, "Closed");
					});
				},
				__("Status")
			);
			if (doc.status != "Stopped" && doc.status != "Completed") {
				frm.add_custom_button(
					__("Stop"),
					function () {
						haloerp.work_order.change_work_order_status(frm, "Stopped");
					},
					__("Status")
				);
				...
				frm.add_custom_button(
					__("Re-open"),
					function () {
						haloerp.work_order.change_work_order_status(frm, "Resumed");
					},
					__("Status")
				);
			}

			haloerp.work_order.setup_stock_reservation(frm);
				...
				if (show_start_btn) {
					...
					if (pending_to_transfer && frm.doc.status != "Stopped") {
						...
						frm.add_custom_button(
							__("Pick List"),
							function () {
								haloerp.work_order.create_pick_list(frm);
							},
							__("Create")
						);
						...
						frm.add_custom_button(
							__("Material Request"),
							function () {
								haloerp.work_order.make_material_request(frm);
							},
							__("Create")
						);
						...
						if (counter > 0) {
							frm.add_custom_button(
								...
								function () {
									...
									haloerp.work_order.make_consumption_se(
										frm,
					...
					if (flt(doc.material_transferred_for_manufacturing) > 0) {
						if (flt(doc.produced_qty) < flt(doc.material_transferred_for_manufacturing)) {
							...
							let finish_btn = frm.add_custom_button(__("Finish"), function () {
								haloerp.work_order.make_se(frm, "Manufacture");
							});
	...
	add_start_button(frm) {
		...
		const start_btn = frm.add_custom_button(__("Start"), () => {
			haloerp.work_order.make_se(frm, "Material Transfer for Manufacture");
		});
	...
	setup_stock_reservation(frm) {
		if (frm.doc.docstatus === 1 && frm.doc.reserve_stock) {
			if (
				...
				frm.add_custom_button(
					__("Reserve"),
					() => haloerp.stock_reservation.make_entries(frm, "required_items"),
			...
			if (frm.events.has_reserved_stock(frm)) {
				if (frm.doc.skip_transfer || frm.doc.material_transferred_for_manufacturing < frm.doc.qty) {
					frm.add_custom_button(
						__("Unreserve"),
						() => haloerp.stock_reservation.unreserve_stock(frm),
						__("Stock Reservation")
					);
				}
				...
				frm.add_custom_button(
					__("Reserved Stock"),
					() => haloerp.stock_reservation.show_reserved_stock(frm, "required_items"),
					__("Stock Reservation")
				);
			}
		}
	...
	set_default_warehouse: function (frm) {
		if (!(frm.doc.wip_warehouse || frm.doc.fg_warehouse)) {
			frappe.call({
				method: "haloerp.manufacturing.doctype.work_order.work_order.get_default_warehouse",
				args: {
					company: frm.doc.company,
				},
				callback: function (r) {
	...
	make_se: function (frm, purpose, qty, is_additional_transfer_entry) {
		if (qty) {
			frappe
				.xcall("haloerp.manufacturing.doctype.work_order.mapper.make_stock_entry", {
					work_order_id: frm.doc.name,
					purpose: purpose,
					qty: qty,
	...
	make_material_request: function (frm, purpose = "Material Transfer for Manufacture") {
		...
		const get_material_request = (for_qty) =>
			frappe.model.open_mapped_doc({
				method: "haloerp.manufacturing.doctype.work_order.mapper.make_material_request",
				frm,
	...
	create_pick_list: function (frm, purpose = "Material Transfer for Manufacture") {
		...
		const get_pick_list = (for_qty) =>
			frappe
				.xcall("haloerp.manufacturing.doctype.work_order.mapper.create_pick_list", {
	...
	make_consumption_se: function (frm, backflush_raw_materials_based_on) {
		...
		frappe.call({
			method: "haloerp.manufacturing.doctype.work_order.mapper.make_stock_entry",
			args: {
				work_order_id: frm.doc.name,
	...
	change_work_order_status: function (frm, status) {
		...
		frappe.call({
			method: `haloerp.manufacturing.doctype.work_order.work_order.${method_name}`,
			freeze: true,
