...
frappe.ui.form.on("Job Card", {
	setup(frm) {
		frm.set_query("operation", () => ({
			query: "haloerp.manufacturing.doctype.job_card.job_card.get_operations",
		...
		if (frm.doc.docstatus === 1 && frm.doc.for_quantity > frm.doc.manufactured_qty) {
			frm.add_custom_button(__("Make Subcontracting PO"), () => {
				frappe.model.open_mapped_doc({
					method: "haloerp.manufacturing.doctype.job_card.mapper.make_subcontracting_po",
					frm: frm,
				});
			}).addClass("btn-primary");
		}
	...
	make_corrective_job_card(frm, operation, for_operation) {
		frappe.call({
			method: "haloerp.manufacturing.doctype.job_card.mapper.make_corrective_job_card",
			args: {
	...
	operation(frm) {
		...
		frappe.call({
			method: "haloerp.manufacturing.doctype.job_card.job_card.get_operation_details",
	...
	make_time_log(frm, args) {
		...
		frappe.call({
			method: "haloerp.manufacturing.doctype.job_card.job_card.make_time_log",
			args: { args },
			freeze: true,
			callback() {
				frm.reload_doc();
	...
	make_material_request(frm) {
		frappe.model.open_mapped_doc({
			method: "haloerp.manufacturing.doctype.job_card.mapper.make_material_request",
			frm: frm,
	...
	make_stock_entry(frm) {
		frappe.model.open_mapped_doc({
			method: "haloerp.manufacturing.doctype.job_card.mapper.make_stock_entry",
			frm: frm,
			run_link_triggers: true,
		});
	},
