...
from haloerp.controllers.website_list_for_contact import (
...
class Customer(TransactionBase):
	...
	if TYPE_CHECKING:
		...
		from haloerp.accounts.doctype.allowed_to_transact_with.allowed_to_transact_with import (
			AllowedToTransactWith,
		)
		from haloerp.accounts.doctype.party_account.party_account import PartyAccount
		from haloerp.selling.doctype.customer_credit_limit.customer_credit_limit import CustomerCreditLimit
		from haloerp.selling.doctype.sales_team.sales_team import SalesTeam
		from haloerp.selling.doctype.supplier_number_at_customer.supplier_number_at_customer import (
			SupplierNumberAtCustomer,
		)
		from haloerp.stock.doctype.company_restriction.company_restriction import CompanyRestriction
		from haloerp.utilities.doctype.portal_user.portal_user import PortalUser
	...
	if credit_limit > 0 and flt(customer_outstanding) > credit_limit:
		...
		if not credit_controller_role or credit_controller_role not in frappe.get_roles():
			...
			frappe.msgprint(
				...
				primary_action={
					"label": "Send Email",
					"server_action": "haloerp.selling.doctype.customer.customer.send_emails",
