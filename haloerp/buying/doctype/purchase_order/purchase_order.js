...
frappe.provide("haloerp.buying");
frappe.provide("haloerp.accounts.dimensions");

cur_frm.cscript.tax_table = "Purchase Taxes and Charges";

haloerp.accounts.taxes.setup_tax_filters("Purchase Taxes and Charges");
haloerp.accounts.taxes.setup_tax_validations("Purchase Order");
haloerp.buying.setup_buying_controller();
...
frappe.ui.form.on("Purchase Order", {
	setup: function (frm) {
		...
		frm.set_query("expense_account", "items", function () {
			return {
				query: "haloerp.controllers.queries.get_expense_account",
	...
	transaction_date(frm) {
		haloerp.buying.prevent_past_schedule_dates(frm);
	...
	get_materials_from_supplier: function (frm) {
		...
		if (po_details && po_details.length) {
			frm.add_custom_button(
				...
				() => {
					frm.call({
						method: "haloerp.controllers.subcontracting_controller.get_materials_from_supplier",
						freeze: true,
...
frappe.ui.form.on("Purchase Order Item", {
	schedule_date: function (frm, cdt, cdn) {
		...
		if (row.schedule_date) {
			if (!frm.doc.schedule_date) {
				haloerp.utils.copy_value_in_all_rows(frm.doc, cdt, cdn, "items", "schedule_date");
			} else {
				set_schedule_date(frm);
			}
		}
...
haloerp.buying.PurchaseOrderController = class PurchaseOrderController extends (
	haloerp.buying.BuyingController
) {
	setup() {
	...
	make_stock_entry() {
		frappe.call({
			method: "haloerp.controllers.subcontracting_controller.make_rm_stock_entry",
			args: {
	...
	make_purchase_receipt() {
		frappe.model.open_mapped_doc({
			method: "haloerp.buying.doctype.purchase_order.mapper.make_purchase_receipt",
			frm: this.frm,
	...
	make_purchase_invoice() {
		frappe.model.open_mapped_doc({
			method: "haloerp.buying.doctype.purchase_order.mapper.make_purchase_invoice",
			frm: this.frm,
		});
	}
	...
	make_subcontracting_order() {
		frappe.model.open_mapped_doc({
			method: "haloerp.buying.doctype.purchase_order.mapper.make_subcontracting_order",
			frm: this.frm,
	...
	add_from_mappers() {
		...
		this.frm.add_custom_button(
			...
			function () {
				haloerp.utils.map_current_doc({
					method: "haloerp.stock.doctype.material_request.mapper.make_purchase_order",
		...
		this.frm.add_custom_button(
			...
			function () {
				haloerp.utils.map_current_doc({
					method: "haloerp.buying.doctype.supplier_quotation.mapper.make_purchase_order",
		...
		this.frm.add_custom_button(
			...
			function () {
				...
				frappe.call({
					method: "haloerp.buying.utils.get_linked_material_requests",
					args: {
						items: my_items,
					},
					callback: function (r) {
...
extend_cscript(cur_frm.cscript, new haloerp.buying.PurchaseOrderController({ frm: cur_frm }));

cur_frm.cscript.update_status = function (label, status) {
	frappe.call({
		method: "haloerp.buying.doctype.purchase_order.purchase_order.update_status",
...
function set_schedule_date(frm) {
	if (frm.doc.schedule_date) {
		haloerp.utils.copy_value_in_all_rows(
			frm.doc,
			frm.doc.doctype,
			frm.doc.name,
			"items",
