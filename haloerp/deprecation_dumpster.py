...
class HaloERPDeprecationError(Warning):
...
class HaloERPDeprecationWarning(Warning):
...
warnings.simplefilter("error", HaloERPDeprecationError)
...
def __get_deprecation_class(graduation: str | None = None, class_name: str | None = None) -> type:
	if graduation:
		...
		class_name = f"{cleaned_graduation}HaloERPDeprecationWarning"
...
### Party starts here
@deprecated(
	"haloerp.controllers.taxes_and_totals.get_itemised_taxable_amount",
	"2024-11-07",
	"v17",
...
@deprecated(
	"haloerp.stock.get_pos_profile_item_details",
	"2024-11-19",
	"v16",
	"Use haloerp.stock.get_pos_profile_item_details_ with a flipped signature",
)
def get_pos_profile_item_details(company, ctx, pos_profile=None, update_data=False):
	from haloerp.stock.get_item_details import get_pos_profile_item_details_

	return get_pos_profile_item_details_(ctx, company, pos_profile=pos_profile, update_data=update_data)


@deprecated(
	"haloerp.stock.get_item_warehouse",
	"2024-11-19",
	"v16",
	"Use haloerp.stock.get_item_warehouse_ with a flipped signature",
)
