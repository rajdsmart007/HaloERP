...
import haloerp
from haloerp.accounts.doctype.accounting_dimension.accounting_dimension import (
	get_accounting_dimensions,
	get_dimensions,
)
from haloerp.accounts.doctype.pricing_rule.utils import (
	apply_pricing_rule_for_free_items,
	apply_pricing_rule_on_transaction,
	get_applied_pricing_rules,
)
from haloerp.accounts.general_ledger import get_round_off_account_and_cost_center
from haloerp.accounts.party import (
	PURCHASE_TRANSACTION_TYPES,
	SALES_TRANSACTION_TYPES,
)
from haloerp.accounts.utils import (
	get_advance_payment_doctypes as _get_advance_payment_doctypes,
)
from haloerp.accounts.utils import get_fiscal_year, validate_fiscal_year
from haloerp.controllers.print_settings import (
	set_print_templates_for_item_table,
	set_print_templates_for_taxes,
)
from haloerp.controllers.sales_and_purchase_return import validate_return
from haloerp.setup.utils import get_exchange_rate
from haloerp.stock.doctype.item.item import get_uom_conv_factor
from haloerp.stock.get_item_details import (
	get_item_details,
)
from haloerp.utilities.regional import temporary_flag
from haloerp.utilities.transaction_base import TransactionBase
...
class AccountsController(TransactionBase):
	...
	def company_currency(self):
		if not hasattr(self, "__company_currency"):
			self.__company_currency = haloerp.get_company_currency(self.company)
	...
	def onload(self):
		...
		if self.is_new():
			...
			if self.doctype in relevant_docs:
				from haloerp.accounts.services.payment_schedule import PaymentScheduleService
	...
	def on_update(self):
		from haloerp.controllers.taxes_and_totals import process_item_wise_tax_details
	...
	def validate(self):
		...
		if self.doctype in ["Sales Invoice", "Purchase Invoice"]:
			if self.is_return:
				...
				from haloerp.accounts.services.deferred_accounting import DeferredAccountingService
		...
		from haloerp.accounts.services.internal_transfer import InternalTransferService
		...
		from haloerp.accounts.services.taxes import TaxService
		...
		from haloerp.accounts.services.party_validation import PartyValidator
	...
	def on_trash(self):
		from haloerp.accounts.utils import delete_exchange_gain_loss_journal
	...
	def calculate_taxes_and_totals(self):
		from haloerp.controllers.taxes_and_totals import calculate_taxes_and_totals
	...
	def validate_due_date(self):
		...
		from haloerp.accounts.party import validate_due_date
			...
			for item in self.get("items"):
				if item.get("item_code"):
					...
					if hasattr(item, "cost_center") and not item.get("cost_center"):
						item.set(
							"cost_center",
							self.get("cost_center") or haloerp.get_default_cost_center(self.company),
						)
	...
	def get_gl_dict(self, args, account_currency=None, item=None):
		from haloerp.accounts.services.base_gl_composer import get_gl_dict

		return get_gl_dict(self, args, account_currency, item)

	def get_voucher_subtype(self):
		from haloerp.accounts.services.base_gl_composer import get_voucher_subtype

		return get_voucher_subtype(self)

	def get_value_in_transaction_currency(self, account_currency, gl_dict, field):
		from haloerp.accounts.services.base_gl_composer import get_value_in_transaction_currency
	...
	def validate_account_currency(self, account, account_currency=None):
		from haloerp.accounts.services.base_gl_composer import validate_account_currency
	...
	def set_advances(self):
		from haloerp.accounts.services.advances import set_advances

		set_advances(self)

	def get_advance_entries(self, include_unallocated=True):
		from haloerp.accounts.services.advances import get_advance_entries
	...
	def validate_advance_entries(self):
		from haloerp.accounts.services.advances import validate_advance_entries

		validate_advance_entries(self)

	def set_advance_gain_or_loss(self):
		from haloerp.accounts.services.advances import set_advance_gain_or_loss
	...
	def gain_loss_journal_already_booked(
		...
		from haloerp.accounts.services.exchange_gain_loss import gain_loss_journal_already_booked
	...
	def make_exchange_gain_loss_journal(
		...
		from haloerp.accounts.services.exchange_gain_loss import make_exchange_gain_loss_journal

		make_exchange_gain_loss_journal(self, args, dimensions_dict)

	def is_payable_account(self, reference_doctype, account):
		from haloerp.accounts.services.exchange_gain_loss import is_payable_account
	...
	def update_against_document_in_jv(self):
		...
		from haloerp.accounts.services.exchange_gain_loss import get_exchange_gain_loss_account
		...
		if lst:
			from haloerp.accounts.utils import reconcile_against_document
	...
	def on_cancel(self):
		from haloerp.accounts.doctype.bank_transaction.bank_transaction import (
	...
	def get_company_default(self, fieldname, ignore_validation=False):
		from haloerp.accounts.utils import get_company_default
	...
	def calculate_total_advance_from_ledger(self):
		from haloerp.accounts.services.advances import calculate_total_advance_from_ledger

		return calculate_total_advance_from_ledger(self)

	def set_total_advance_paid(self):
		from haloerp.accounts.services.advances import set_total_advance_paid

		set_total_advance_paid(self)

	def set_advance_payment_status(self):
		from haloerp.accounts.services.advances import set_advance_payment_status
	...
	def delink_advance_entries(self, linked_doc_name):
		from haloerp.accounts.services.advances import delink_advance_entries
	...
	def create_advance_and_reconcile(self, party_link):
		from haloerp.accounts.services.advances import create_advance_and_reconcile

		create_advance_and_reconcile(self, party_link)

	def check_conversion_rate(self):
		default_currency = haloerp.get_company_currency(self.company)
	...
	def check_if_fields_updated(self, fields_to_check, child_tables):
		from haloerp.accounts.services.child_item_update import check_if_child_table_updated
	...
	def set_transaction_currency_and_rate_in_gl_map(self, gl_entries: list) -> None:
		from haloerp.accounts.services.exchange_gain_loss import set_transaction_currency_and_rate_in_gl_map
...
from haloerp.accounts.services.payment_schedule import (
	get_discount_date,
	get_due_date,
...
from haloerp.accounts.services.child_item_update import update_child_qty_rate
...
from haloerp.accounts.services.base_gl_composer import (
